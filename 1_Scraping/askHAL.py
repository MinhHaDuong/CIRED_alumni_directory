...
def final_normalize_name(name):
    ...
    surname, which caused ``Jean Dupont`` and ``Dupont Jean`` to produce
    different keys.  To make matching more robust we now compute both
    ``surname first`` and ``surname last`` forms and return the
    alphabetically earliest.  This mirrors the logic used in ``merge.py``.
    """
